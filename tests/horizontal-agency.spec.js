...
test("paired vertical/lateral drivers publish a deterministic agency baseline", async ({ page }) => {
  ...
  const report = await page.evaluate(({ modes, difficulties, seeds, decisionBuffer }) => {
  ...
  }, { modes: MODES, difficulties: DIFFICULTIES, seeds: SEEDS, decisionBuffer: DRIVER_DECISION_BUFFER_SECONDS });
